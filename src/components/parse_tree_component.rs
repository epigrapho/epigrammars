...
pub fn ParseTreeComponent(
    ...
    let get_style = |name: &str, production_hues: Arc<HashMap<String, f32>>| -> String {
        let hue = production_hues
            .get(&format!("<{}>", name))
            .unwrap_or(&0.0)
            .to_owned();
        format!(
            "flex: {}; background-color: hsl({}deg, 16%, 50%)",
            weight, hue
        )
    };

    view! {
        <div class=style::non_terminal_node style=get_style(&lhs_name, production_hues.clone())>
